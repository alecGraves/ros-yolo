...
class videosub():
    def __init__(self, topic, shape=(416, 416)):
        rospy.Subscriber(topic, UInt8MultiArray, self.callback)
        self.eval_shape = shape
        self.topic = topic
        self.image = None
    ...
    def getProcessedImage(self):
        self.newImgAvailable = False
        return np.expand_dims(np.array(cv2.resize(self.image, self.eval_shape), dtype='float32')*0.003921568, axis=0)
